...
vector<Point> find2Line(Mat img, Mat imgROI, vector<Point> lines);
...
int main(int argc, char** argv)
{
	...
	while (1) {
		...
		//finalLines = find2Line(img, imgROI, lines);
...
//vector<Vec2f> findEdgeLines(Mat original, Mat img, int lowThres, int highThres, const char* windowName)
{
	...
	*/
	...
	for (size_t i = 0; i < lines.size(); i++)
	{
		...
		{ // 수직 행
			line(original, Point(l[0], l[1]), Point(l[2], l[3]), Scalar(0, 0, 255), 1, 8);
...
Mat houghLineImage(Mat img, Mat imgROI, vector<Point> red_lines) {
	...
	// houghLinesP 선분들을 흰선으로 그리기
	Mat back1 = img.clone();
	back1 = Scalar::all(0);

	for (int i = 0; i < red_lines.size(); i+=2) {

		line(back1, red_lines[i], red_lines[i+1], Scalar::all(255), 1, 8);
		//line(back, lines[2], lines[3], Scalar::all(255), 1, 8);

	}

	Mat contours = back1.clone();
	Canny(back1, contours, 70, 210, 3);
	...
	//// 선 그리기
	//cv::Mat result(back.rows, back.cols, CV_8U, cv::Scalar(255));
	////std::cout << "Lines detected: " << lines.size() << std::endl;
	...
	//		cv::Point pt2((rho - result.rows * sin(theta)) / cos(theta), result.rows);
	//		// 마지막 행에서 해당 선의 교차점
	//		cv::line(back, pt1, pt2, cv::Scalar(255), 1); // 하얀 선으로 그리기
	//	}
	//	//else { // 수평 행
	//	//	cv::Point pt1(0, rho / sin(theta)); // 첫 번째 열에서 해당 선의 교차점
	//	//	cv::Point pt2(result.cols, (rho - result.cols * cos(theta)) / sin(theta));
	...
	namedWindow("houghLines_white");
	imshow("houghLines_white", back1);
...
vector<Point> findMeanLine()
{
	queue<Point> tmp1(lftq1);
	queue<Point> tmp2(lftq2);
...
Mat drawLines(Mat img, vector<Point> lines)
{
	line(img, lines[0], lines[1], Scalar(0, 0, 255), 1, 8);
	line(img, lines[2], lines[3], Scalar(0, 0, 255), 1, 8);
